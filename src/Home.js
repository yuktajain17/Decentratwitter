...
const Home = ({contract, account}) => {
    ...
    const loadPosts = async () => {
        ...
        //and if yes then set "setHasProfile" to true
        ...
        let posts = await Promise.all(results.map(async i => {
            //using hash to fetch the post's metadata stored on ipfs
            let response = await fetch(`https://ipfs.infura.io/ipfs/${i.hash}`)
            const metadataPost = await response.json()
            //getting author's NFT profile
            const nftId = await contract.profiles(i.author)
            //getting URI URL of the NFT profile
            const uri = contract.tokenURI(nftId)
            //fetching NFT profile metadata
            //const metadataProfile = await response.clone().json()
            //defining author object
            const author = {
                address: i.author,
                //username: metadataProfile.username,
                //avatar: metadataProfile.avatar
                username: metadataPost.username,
                avatar: metadataPost.avatar
            } 
                        ...
                        <div key={key} className="col-lg-12 my-3 mx-auto" style={{ width: '1000px' }}>
                            <Card border="primary">
                                ...
                                <Card.Footer className="list-group-item">
                                    ...
                                    {address === post.author.address || !hasProfile ? //you can tip posts if ypu have an NFT and you are not author of that post
